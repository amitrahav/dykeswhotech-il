...
export function Navigation({ variant = 'gradient', empty = false, text = 'dark' }: NavigationProps) {
    ...
    return (
        ...
        >
            {!empty && (
                <>
                    <Link to="/" className={`hover:text-primary hover:border-b-[2px] hover:pb-0 transition-colors ${text === 'bright' ? 'text-white' : 'text-primary pb-0 border-b-[1px] border-black'}`}>{navigation.home}</Link>
                    <span className={`hidden sm:inline-block w-[3px] h-[3px] rounded-full ${text === 'bright' ? 'bg-white' : 'bg-primary'}`}></span>
                    <Link to="/about" className={`hover:text-primary hover:border-b-[2px] hover:pb-0 transition-colors ${text === 'bright' ? 'text-white' : 'text-primary pb-0 border-b-[1px] border-black'}`}>{navigation.about}</Link>
                    <span className={`hidden sm:inline-block w-[3px] h-[3px] rounded-full ${text === 'bright' ? 'bg-white' : 'bg-primary'}`}></span>
                    <Link to="/sponsorship" className={`hover:text-primary hover:border-b-[2px] hover:pb-0 transition-colors ${text === 'bright' ? 'text-white' : 'text-primary pb-0 border-b-[1px] border-black'}`}>{navigation.sponsorship}</Link>
                    <span className={`hidden sm:inline-block w-[3px] h-[3px] rounded-full ${text === 'bright' ? 'bg-white' : 'bg-primary'}`}></span>
